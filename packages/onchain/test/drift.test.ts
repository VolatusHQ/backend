 ...
 *
 * `apps/web/app/app/lib/onchain/` is still a separate, TypeScript-only copy of
 * the same chains/addresses/constants this package exports — the frontend has
 * not been repointed at `@volatus/onchain` yet. Until it is, the two trees can
 * silently drift: someone fixes an address here and forgets there, or the
 * reverse, and the result is not an error, it is a plausible wrong number.
 *
 * This file cannot `import` the web copy — it lives in a different package
 * and is not built as a library — so it reads the web source files as *text*
 * and greps them with the same regex a human would use to spot-check by eye.
 * That is deliberately unsophisticated: the point is to catch a literal typo
 * or a forgotten edit, not to parse TypeScript.
 *
...
const webOnchainDir = resolve(__dirname, "../../../apps/web/app/app/lib/onchain");
 ...
 *
 * The `SIGMA_STREAM` exemption that used to live here is gone: `apps/web` was
...
describe("apps/web onchain copy does not disagree with @volatus/onchain", () => {
  ...
  it("neither tree points at the dead SigmaStream", () => {
    ...
    expect(
      ...
      "apps/web has drifted back to the dead SigmaStream",
  ...
  for (const [name, webValue] of extractAddressLiterals(webAddressesSrc)) {
    it(`${name} is byte-identical between apps/web and @volatus/onchain`, () => {
      const pkgValue = (onchainAddresses as Record<string, unknown>)[name];
      expect(
        typeof pkgValue,
        `@volatus/onchain does not export a constant named ${name}, but apps/web does`,
      ).toBe("string");
      expect(
        pkgValue,
        `${name} has diverged: apps/web has ${webValue}, @volatus/onchain has ${pkgValue}`,
      ).toBe(webValue);
    });
  }
  ...
  it("chain ids 1301 (Unichain Sepolia) and 5042002 (Arc Testnet) match both trees", () => {
    ...
    expect(webIds, "apps/web chains.ts must define Unichain Sepolia's id 1301").toContain(1301);
    expect(webIds, "apps/web chains.ts must define Arc Testnet's id 5042002").toContain(5042002);
